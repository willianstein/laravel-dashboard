...
function loadDataTableWithButtons(url,tableId){
    $('#'+tableId).DataTable({
        pageLength: 100,
        dom: 'Bfrtip',
        buttons: [
...
$(document).on('click','.ajax-link', function(e){
    e.preventDefault();
    $('.ajax-load').fadeIn().css("display", "flex");
    ...
    if(actions){
        $.each(actions,function(action,value) {
            ...
            switch (action) {
                ...
                case 'clearForm':
                    $(':input',obj)
                        .not(':button, :submit, :reset, [name=\'_token\']')
                        .val('')
                        .removeAttr('checked')
                        .removeAttr('selected');
                    break;
                ...
                case 'loadForm':
                    ...
                    $.each(data, function (k,v){
                        if($('#'+value+' #'+k).length == 1){
                            /* TEXT */
                            if($('#'+value+' #'+k)[0].type == 'text' || $('#'+value+' #'+k)[0].type == 'email' || $('#'+value+' #'+k)[0].type == 'number' || $('#'+value+' #'+k)[0].type == 'date' || $('#'+value+' #'+k)[0].type == 'hidden' || $('#'+value+' #'+k)[0].type == 'select-one'){
                                $('#'+value+' #'+k).val(v);
                            }
                            /* CHECKBOX */
                ...
                case 'drawDataTable':
                    console.log('Povoando DataTable: ' + value);
                    let myTable = $('#'+value).DataTable({
                        order: [],
                        pageLength: 100,
                        dom: 'Bfrtip',
                        data: data.data,
